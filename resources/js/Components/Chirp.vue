...
const form = useForm({
    ...
    media: null, // Jangan sertakan media saat inisialisasi form
});
...
const handleFileChange = (event) => {
    ...
    if (file) {
        form.setData("media", file); // Menambahkan file ke form data
    }
};
...
// Fungsi untuk memeriksa apakah file adalah video
const isVideo = (media) =>
    media &&
    (media.endsWith(".mp4") ||
        media.endsWith(".avi") ||
...
const getMediaUrl = (media) => `/storage/${media}`;
</script>
...
<template>
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            ...
            <!-- Menampilkan pesan chirp -->
            <p v-else class="mt-4 text-lg text-gray-900">{{ chirp.message }}</p>

            <!-- Menampilkan media (gambar/video) jika ada -->
            <div v-if="chirp.media" class="mt-4">
                <div v-if="isImage(chirp.media)">
                    <img :src="getMediaUrl(chirp.media)" alt="Uploaded Image" />
                </div>
                <div v-else-if="isVideo(chirp.media)">
                    <video
                        controls
                        class="w-full max-h-96 object-cover rounded-lg"
                    >
                        <source
                            :src="getMediaUrl(chirp.media)"
                            type="video/mp4"
                        />
                        Video not supported.
                    </video>
